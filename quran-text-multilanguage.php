...
/**
 * On first install: create sura lookup table + set default options (labels, colors, reciter, etc.).
 */
...
/* Default options (only used when option does not exist). */
...
add_option('quran_recitator', 'Maher_al_me-aqly', '', 'yes');
	...
	add_option('quran_dark_mode', 'no', '', 'yes');
...
/** Admin-only scripts (e.g. jscolor for options). */
...
/** Register all plugin options with Settings API. */
function qtm_registeroptions() {
	...
	register_setting( 'quran-options', 'quran_dark_mode');
...
/** Front: enqueue jQuery, load_sura.js (with nonce for lazy load), soundmanager + player. */
function add_js_scripts() {
	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'loadquran', plugin_dir_url(__FILE__).'/js/load_sura.js', array('jquery'), '1.0', true );
	wp_localize_script( 'loadquran', 'qtm_ajax', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'verses_nonce' => wp_create_nonce( 'qtm_load_verses' ),
	) );
...
add_action( 'wp_ajax_qtm_load_verses_chunk', 'qtm_load_verses_chunk' );
add_action( 'wp_ajax_nopriv_qtm_load_verses_chunk', 'qtm_load_verses_chunk' );

require_once 'inc/functions_quran.php';

/** Shortcode [quran]: inject styles, markup, and init. */
