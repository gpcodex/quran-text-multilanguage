<?php
/**
 * Front-end markup for [quran] shortcode. Skipped for REST to avoid output in API responses.
 */
if ( ! defined( 'REST_REQUEST' ) ) :

?>
<div id="template_quran"<?php echo ( get_option( 'quran_dark_mode' ) === 'yes' ) ? ' class="quran-dark-mode"' : ''; ?>>
<div id="quran_main">
<div id="kb_select_quran">
<form class="aya1">
<select name="sourate" id="change_sura" class="quran-select aya2">
<option disabled="disabled" selected="selected"><?php echo esc_html( get_option( 'quran_changesuratxt' ) ); ?></option>
<?php
	global $wpdb;
...

	{
		...
		echo '<option value="?sourate='.esc_attr( trim($name_sourate[1]) ).'-' . intval($sourate->nom_id) . '"';
	if(isset($sura)){
...
<div id="kb_select_language">
		...
		<select name="select_language" id="select_language" class="quran-select">
			<option disabled="disabled" selected="selected"><?php echo esc_html( get_option( 'quran_changelangtxt' ) ); ?></option>				
...
<?php
	...
	// URL has sura + lang → show that sura and sync history
	if ( isset( $_GET['sourate'] ) && isset( $_GET['lang'] ) ) {
		if ( preg_match( '/[0-9]{1,3}$/', sanitize_text_field( wp_unslash( $_GET['sourate'] ) ), $matches ) ) {
			$sura = $matches[0];
			?>
			<script>
			document.addEventListener('DOMContentLoaded', function() {
				var sourate = "<?php echo esc_js(sanitize_text_field($sura)); ?>";
				var lang = "<?php echo esc_js(sanitize_text_field($_GET['lang'] ?? '')); ?>";
			...
			echo '<div class="error">Invalid surah number</div>';
		}
	} else {
		// No sura in URL → show Al-Fatiha with default language
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    history.pushState({ path: window.location.pathname }, '', UrlPrevSourate + '&lang=' + lang);
});
</script>
